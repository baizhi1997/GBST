...
struct SurvTreeParam : public XGBoostParameter<SurvTreeParam> {
  int num_class;
  // declare parameters
  DMLC_DECLARE_PARAMETER(SurvTreeParam) {
    DMLC_DECLARE_FIELD(num_class).set_lower_bound(1)
      .describe("Number of output class in the multi-class classification.");
  }
};

class SurvTreeObj : public ObjFunction {
public:
  explicit SurvTreeObj(bool output_prob = false)
    : output_prob_(output_prob) {}
  ...
  void GetGradient(const HostDeviceVector<bst_float>& preds,
    ...
    CHECK_NE(info.labels_.Size(), 0U) << "label set cannot be empty";
    CHECK(preds.Size() == (static_cast<size_t>(param_.num_class) * info.labels_.Size()))
      << "RawMultiClassObj: label size and pred size does not match.\n"
      << "label.Size() * num_class: "
      << info.labels_.Size() * static_cast<size_t>(param_.num_class) << "\n"
      << "num_class: " << param_.num_class << "\n"
      << "preds.Size(): " << preds.Size();
    ...
    common::Transform<>::Init(
      ...
      for (int k = 0; k < nclass; ++k) {
        // Computation duplicated to avoid creating a cache.
        bst_float hazard = 1.0f / (1.0f + expf(-point[k]));
        bst_float h = (1.0f - hazard) * hazard;
        bst_float grad = k <= label - 1 ? hazard : hazard - 1.0f;
        grad = k >= label + 1 ? 0 : grad;
        h = k >= label + 1 ? 0 : h;
        gpair[idx * nclass + k] = GradientPair(grad*wt, h);
      }
    }, common::Range{ 0, ndata }, device, false)
      .Eval(out_gpair, &info.labels_, &preds, &info.weights_, &label_correct_);
    ...
    for (auto const flag : label_correct_h) {
      if (flag != 1) {
        LOG(FATAL) << "SurvTreeObj: label must be in [0, num_class).";
      }
    }
  }
  void PredTransform(HostDeviceVector<bst_float>* io_preds) override {
    this->Transform(io_preds);
  }
  void EvalTransform(HostDeviceVector<bst_float>* io_preds) override {
    this->Transform(io_preds);
  }
  ...
  inline void Transform(HostDeviceVector<bst_float> *io_preds) {
    const int nclass = param_.num_class;
    const auto ndata = static_cast<int64_t>(io_preds->Size() / nclass);
    max_preds_.Resize(ndata);
  ...
  void SaveConfig(Json* p_out) const override {
    auto& out = *p_out;
    out["name"] = String("multi:survtree");
    
    out["surv_tree_param"] = toJson(param_);
  }

  void LoadConfig(Json const& in) override {
    fromJson(in["surv_tree_param"], &param_);
  }

private:
  // output probability should be forced.
  bool output_prob_ = false;
  // parameter
  SurvTreeParam param_;
  // Cache for max_preds
  HostDeviceVector<bst_float> max_preds_;
  HostDeviceVector<int> label_correct_;
};
...
DMLC_REGISTER_PARAMETER(SurvTreeParam);

XGBOOST_REGISTER_OBJECTIVE(SurvTree, "multi:survtree")
.describe("Gradient Boosting Survival Tree mod for xgboost. output survival-rate evaluation f(t). See paper.")
.set_body([]() { return new SurvTreeObj(); });
