...
class SurvBoostError(Exception):
    """Error thrown by survboost trainer."""
...
def _load_lib():
    ...
    if lib.XGBRegisterLogCallback(lib.callback) != 0:
        raise SurvBoostError(lib.XGBGetLastError())
    return lib
...
def _check_call(ret):
    ...
    """
    if ret != 0:
        raise SurvBoostError(py_str(_LIB.XGBGetLastError()))
