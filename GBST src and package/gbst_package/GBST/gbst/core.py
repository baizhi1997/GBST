...
class gbstError(Exception):
    """Error thrown by gbst trainer."""
...
def _load_lib():
    ...
    if not lib_success:
        ...
        raise gbstError(
            'Altered XGBoost Library ({}) could not be loaded.\n'.format(libname) +
            'Likely causes:\n' +
    ...
    if lib.XGBRegisterLogCallback(lib.callback) != 0:
        raise gbstError(lib.XGBGetLastError())
    return lib
...
def _check_call(ret):
    ...
    """
    if ret != 0:
        raise gbstError(py_str(_LIB.XGBGetLastError()))
...
class DMatrix(object):
    """Data Matrix used in GBST.

    DMatrix is a internal data structure that used by GBST
...
class Booster(object):
    ...
    Booster is the model of gbst/xgboost, that contains low level routines for
    ...
    def update(self, dtrain, iteration):
